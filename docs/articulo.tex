...
AI-assisted agent in preventive and reactive modes.
...
A pesar de la urgencia de este problema, persisten brechas concretas
...
reales~\cite{morsy2022,sakhawat2019}. Una tercera brecha operativa
consiste en la ausencia de interfaces que traduzcan los resultados de
detección y priorización en flujos de trabajo utilizables por analistas
de seguridad, más allá de métricas desacopladas del contexto de red.
...
artículo propone un \textit{autoencoder} no supervisado diseñado para
...
exclusivamente para la evaluación posterior del rendimiento. La
Fig.~\ref{fig:pipeline} resume la arquitectura de extremo a extremo.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figs/pipeline_architecture.png}
\caption{Arquitectura del sistema: captura pasiva, extracción L2,
\textit{autoencoder} con calibración de $\theta$, priorización de
alertas y plataforma de operación \textit{ARPegio}.}
\label{fig:pipeline}
\end{figure}
...
7 & $\sim$2 min & Reconocimiento ARP & \textit{arping} en Kali \\
\hline
\end{tabular}
\end{table}
...
(media, desviación estándar, mínimo y máximo); (vi)~ratio de
tramas broadcast; y (vii)~conteo total de paquetes por ventana. Esta
selección cubre directamente las familias de
...
\begin{tabular}{@{}l r >{\raggedright\arraybackslash}p{0.42\columnwidth}@{}}
\hline
\textbf{Clase} & \textbf{$n$} & \textbf{Señal L2 dominante} \\
\hline
Tráfico normal & 116 & ICMP baseline \\
ARP spoofing & 6 & ARP alto, bajo req./reply \\
MAC flooding & 795 & 50 MACs únicas, entropía $\approx 3{,}91$ \\
Escaneo de puertos & 36 & Longitud de trama uniforme \\
Reconocimiento ARP & 6 & ARP rate $=1{,}0$, sondeo \\
\hline
\end{tabular}
\end{table}
...
en entrenamiento. Dado el desbalance extremo del \textit{dataset}
completo (795 ventanas de \macflood{} frente a 116 de
\trafonormal), se aplicó un submuestreo aleatorio de \macflood{} a
116 ventanas en total---con semilla fija para reproducibilidad---antes
de separar por fuente de captura. De ese subconjunto, 62 ventanas
pertenecen a la \victimados{} y forman el bloque de inundación MAC del
conjunto de prueba, junto con las 75 ventanas normales y las clases
minoritarias de la misma perspectiva. Así se evita que las métricas
queden dominadas por la ráfaga de \macflood{} sin renunciar a evaluar
ataques reales de la captura. El optimizador empleado es Adam
...
cuatro componentes funcionales: entrenamiento del \textit{autoencoder},
motor de priorización heurística, emisión de alertas ordenadas y
plataforma web de operación. El cuarto componente---\textit{ARPegio},
descrito en la Sección~V---expone métricas, alertas y escenarios de
laboratorio al analista sin depender del entorno de entrenamiento.
...
La Tabla~\ref{tab:priorizacion_ejemplo} muestra alertas representativas
de la \victimados{} con los valores de $s$, $f$, $i$ y $P$ calculados
por la ecuación~\eqref{eq:priority}.

\begin{table}[htbp]
\caption{Ejemplo de priorización de alertas (\victimados, $\theta=0{,}114$, $W=10$)}
\label{tab:priorizacion_ejemplo}
\centering
...
\hline
\textbf{\#} & \textbf{Amenaza inferida} & \textbf{Etiqueta real} & $\boldsymbol{s}$ & $\boldsymbol{f}$ & $\boldsymbol{i}$ & $\boldsymbol{P}$ \\
\hline
1 & MAC flood & MAC flood & 1.00 & 1.00 & 1.0 & 1.00 \\
2 & MAC flood & MAC flood & 1.00 & 0.90 & 1.0 & 0.97 \\
3 & ARP spoof & ARP spoof & 1.00 & 0.30 & 0.9 & 0.77 \\
4 & Recon. ARP & Recon. ARP & 1.00 & 0.10 & 0.6 & 0.65 \\
5 & Desconocida & Normal (FP) & 1.00 & 0.10 & 0.5 & 0.63 \\
\hline
\end{tabular}%
}
\end{table}

Las filas 1--2 ilustran una ráfaga sostenida de \macflood{} con
$f \rightarrow 1$. La fila 3 corresponde a \arpspoof{} con menor
persistencia ($f=0{,}3$) pero impacto elevado ($i=0{,}9$). La fila 5
documenta un falso positivo sobre \trafonormal{} cuya prioridad
relativamente alta ($P=0{,}63$) refleja severidad máxima pese a baja
frecuencia e impacto moderado.

\subsection{Validación operativa de la plataforma}
Los resultados cuantitativos anteriores se contrastan en la interfaz
descrita en la Sección~V. La vista \textbf{Monitor} reproduce el
AUC-ROC de 0{,}999, el umbral $\theta$ calibrado, el volumen de alertas
y la tasa de falsos positivos del 4\%, junto con un histograma
equivalente a la Fig.~\ref{fig:errors}; un control de percentiles permite
explorar el trade-off TN/FP/FN/TP sin reentrenar. La vista
\textbf{Alertas} ordena las 71 ventanas anómalas por puntaje $P$ y
destaca las tres de mayor prioridad, coherente con el comportamiento del
motor de priorización (\macflood{} con $P=1{,}0$ frente a eventos ARP
menos persistentes). La vista \textbf{Laboratorio} reproduce la decisión
binaria del \textit{autoencoder} en el cliente y documenta de forma
explícita el falso negativo de \portscan{} desde la \victimados{}
(Fig.~\ref{fig:platform_lab}). El módulo \textbf{Agente} complementa
esta validación con asistencia preventiva y reactiva, accesible de forma
contextual desde las demás vistas (Figs.~\ref{fig:platform_monitor},
\ref{fig:platform_alerts} y~\ref{fig:platform_agent}). En conjunto, la
plataforma confirma que las métricas del experimento son trazables y
accionables para el analista, sin sustituir la evaluación estadística
de las subsecciones precedentes.
...
0{,}999, detectando por completo \macflood, \arpspoof{} y
