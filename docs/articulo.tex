...
(\texttt{mac\_flood}: 1.0; \texttt{arp\_spoof}: 0.9; \texttt{port\_scan}:
0.7; \texttt{arp\_recon}: 0.6; desconocido: 0.5), sin emplear etiquetas
...
(i)~si \texttt{unique\_src\_macs} $\geq 20$, se clasifica como
\texttt{mac\_flood}; (ii)~si no, y \texttt{arp\_rate} $\geq 0{,}5$, entonces
\texttt{arp\_req\_reply\_ratio} $< 1$ indica \texttt{arp\_spoof} y, en caso
contrario, \texttt{arp\_recon}; (iii)~si no, y \texttt{frame\_len\_std} $< 5$,
se asigna \texttt{port\_scan}; (iv)~en cualquier otro caso, la alerta queda
...
33 ventanas \texttt{normal} de \texttt{victima1}; ocho ventanas normales
restantes de la misma fuente calibran $\theta$ (percentil~95). La
evaluación se realiza sobre 144 ventanas de \texttt{victima2} no vistas
en entrenamiento, con submuestreo de \texttt{mac\_flood} a 116 ventanas
para equilibrar la comparación por clase, igualando así el número de
ventanas de la clase dominante al total de ventanas \texttt{normal}
...
de priorización se fijó en $W = 10$. La implementación de referencia se
distribuye en el repositorio como \texttt{scripts/train\_autoencoder.py},
\texttt{scripts/prioritizer.py} y \texttt{scripts/prioritize\_alerts.py}.
...
$3{,}9\times10^{-2}$ sobre 33 ventanas normales de
\texttt{victima1}. La Fig.~\ref{fig:loss} muestra la curva de
...
sobre ocho ventanas normales de validación (\texttt{victima1}, no
vistas en entrenamiento). La Fig.~\ref{fig:errors} contrasta la
distribución de errores normales frente a ventanas de ataque en
\texttt{victima2}: las familias ARP y \texttt{mac\_flood} quedan
...
ataques en \texttt{victima2}; línea vertical en $\theta$.}
\label{fig:errors}
\end{figure}

\subsection{Detección binaria y AUC}
Sobre 144 ventanas de prueba (\texttt{victima2}), la clasificación
binaria \texttt{normal}/\texttt{ataque} alcanzó precisión, \textit{recall}
...
alcanzaron detección perfecta en \texttt{victima2}. No obstante, dado
que \texttt{arp\_spoof} cuenta con únicamente tres ventanas en el
conjunto de prueba, este resultado debe interpretarse con cautela
estadística. La ausencia de detección en \texttt{port\_scan} (0/1)
refleja la baja visibilidad de este ataque desde la perspectiva de
\texttt{victima2}, donde solo una ventana fue etiquetada para esta
...
795 de \texttt{mac\_flood}).

\begin{table}[htbp]
\caption{Detección por familia de ataque (\texttt{victima2}, $\theta=0{,}114$)}
\label{tab:deteccion_clase}
\centering
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lrrr}
\hline
\textbf{Etiqueta} & \textbf{Detectadas} & \textbf{Total} & \textbf{Tasa (\%)} \\
\hline
\texttt{mac\_flood}  & 62 & 62 & 100.0 \\
\texttt{arp\_spoof}  &  3 &  3 & 100.0 \\
\texttt{arp\_recon}  &  3 &  3 & 100.0 \\
\texttt{port\_scan}  &  0 &  1 &   0.0 \\
\texttt{normal} (FP) &  3 & 75 &   4.0 \\
\hline
\end{tabular}
\end{table}
...
alertas generadas en \texttt{victima2}---62 \texttt{mac\_flood}, tres
\texttt{arp\_spoof}, tres \texttt{arp\_recon} y tres falsos positivos
sobre tráfico \texttt{normal}---las ventanas de \texttt{mac\_flood}
...
El tamaño reducido de clases minoritarias (\texttt{arp\_spoof},
\texttt{arp\_recon}, \texttt{port\_scan}) limita la generalización
estadística. La validación cruzada por víctima (\texttt{victima1} $\to$
\texttt{victima2}) demuestra transferencia entre perspectivas de captura,
pero no sustituye un despliegue en producción con mayor diversidad
topológica. La integración del agente de mitigación (modos preventivo y
reactivo) queda como extensión futura del \textit{pipeline}.
...
sin muestras de ataque. Sobre tráfico de \texttt{victima2} no visto en
entrenamiento, el sistema alcanzó 97\% de exactitud binaria y AUC-ROC de
0.998, detectando por completo \texttt{mac\_flood}, \texttt{arp\_spoof} y
\texttt{arp\_recon}---si bien \texttt{arp\_spoof} y \texttt{arp\_recon}
cuentan con muestras de prueba reducidas ($n=3$ cada una)---, con una
tasa de falsos positivos del 4\% sobre tráfico normal.

La tercera contribución---motor de priorización por severidad, frecuencia
e impacto---cierra el ciclo operativo al ordenar 71 alertas sin
intervención humana ni etiquetas en inferencia. Los hallazgos confirman
que el monitoreo pasivo anclado en tramas IEEE~802.3 es viable para
alertar tempranamente sobre ataques de suplantación y saturación en LAN
institucionales simuladas. Como líneas futuras se identifican: ampliar
capturas para \texttt{port\_scan} y clases minoritarias, desplegar el
agente de mitigación con IA generativa, y validar el motor de
priorización con roles heterogéneos (servidor/cliente/IoT) en topologías
más amplias.
